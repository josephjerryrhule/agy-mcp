...
${options.instructions}`;
    ...
    const args = [];
    if (options.threadId) {
        args.push('exec', 'resume', '--json', '--dangerously-bypass-approvals-and-sandbox', '-m', modelToUse);
        ...
        args.push('exec', '--json', '--dangerously-bypass-approvals-and-sandbox', '-C', cwd, '-m', modelToUse);
        if (options.reasoningEffort) {
            args.push('-c', `model_reasoning_effort="${options.reasoningEffort}"`);
        }
        if (options.sandbox) {
